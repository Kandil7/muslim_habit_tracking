...
class BadgeRepositoryImpl implements BadgeRepository {
  ...
  @override
  Future<Either<Failure, List<Badge>>> getBadgesByCategory(
    String category,
  ) async {
    try {
  ...
  @override
  Future<Either<Failure, Badge>> updateBadgeProgress(
    String id,
    int progress,
  ) async {
    try {
  ...
  Future<Either<Failure, bool>> checkBadgeRequirements(
    ...
    try {
      ...
      return badgeResult.fold((failure) => Left(failure), (badge) {
            ...
            if (userValue is int && requiredValue is int) {
              if (userValue >= requiredValue) {
                ...
                final requirementProgress =
                    (userValue / requiredValue * 100).round();
              ...
              if (userValue.length >= requiredValue) {
                ...
                // Calculate progress percentage
                final requirementProgress =
                    (userValue.length / requiredValue * 100).round();
                progress += requirementProgress;
              }
              totalRequirements++;
            }
        ...
        final averageProgress =
            totalRequirements > 0 ? (progress / totalRequirements).round() : 0;
