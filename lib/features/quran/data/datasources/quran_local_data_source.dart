...
class QuranLocalDataSourceImpl implements QuranLocalDataSource {
    ...
    try {
      ...
      for (int i = 0; i < Constants.quranList.length; i++) {
        ...
        surahs.add(
          QuranModel(
            id: i + 1,
            name: surah['enName'] as String,
            arabicName: surah['arName'] as String,
            englishName: surah['enName'] as String,
            revelationType: surah['enType'] as String,
            numberOfAyahs: surah['verses'] as int,
            startPage: surah['start'] as int,
            isBookmarked: false,
          ),
        );
      }
  ...
  Future<List<QuranReadingHistoryModel>> getReadingHistory() async {
    try {
      ...
      throw CacheException(
        message: 'Failed to get reading history: ${e.toString()}',
      );
    }
  }
  ...
  Future<QuranReadingHistoryModel> addReadingHistory(
    ...
    try {
      ...
      throw CacheException(
        message: 'Failed to add reading history: ${e.toString()}',
      );
    }
  }
  ...
  Future<bool> clearReadingHistory() async {
    try {
      ...
      throw CacheException(
        message: 'Failed to clear reading history: ${e.toString()}',
      );
    }
  }
  ...
  Future<QuranReadingHistoryModel?> getLastReadPosition() async {
    try {
      ...
      throw CacheException(
        message: 'Failed to get last read position: ${e.toString()}',
      );
    }
  }
  ...
  Future<bool> updateLastReadPosition(QuranReadingHistoryModel history) async {
    try {
      ...
      await quranBox.put('last_read_position', jsonEncode(newHistory.toJson()));
      ...
      throw CacheException(
        message: 'Failed to update last read position: ${e.toString()}',
      );
    }
  }
